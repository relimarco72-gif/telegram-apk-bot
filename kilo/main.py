...
# ⚠️ غيّر هذا الرابط إلى رابط استضافتك الفعلي
WEBAPP_URL = os.getenv("WEBAPP_URL", "https://white-flossi-33.tiiny.site")
...
# ══════════════════════════════════════════════════════════════════
#  نظام الفتح الجماعي عبر إعلانات Monetag
# ══════════════════════════════════════════════════════════════════

async def handle_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    معالجة البيانات الواردة من تطبيق Telegram المصغر (Monetag Web App).
    يتم استدعاؤها عندما يرسل المستخدم بيانات عبر Telegram.WebApp.sendData.
    """
    user_id = update.effective_user.id
    ...
    try:
        ...
        logger.warning("Invalid web_app_data from user %s", user_id)
        return

    action = payload.get("action")
    if action != "monetag_reward_done":
        return
    ...
    # رسالة شكر للمستخدم
    views_progress = create_progress_bar(current_views, required_views)
    await update.message.reply_text(
        f"🎬 شكراً لمساهمتك!\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📹 المشاهدات: {current_views}/{required_views}\n"
        f"📊 التقدم: {views_progress}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━",
    )
