...
WEBAPP_URL = os.getenv("WEBAPP_URL", "https://peach-dianna-54.tiiny.site")
...
async def _handle_view_completed(update: Update, context: ContextTypes.DEFAULT_TYPE, watch_token: str) -> None:
    ...
    # التحقق من أن الوقت المنقضي الفعلي على الخادم هو 29 ثانية على الأقل
    time_elapsed = time.time() - session["start_time"]
    if time_elapsed < 29:
        remaining = int(30 - time_elapsed)
        await update.message.reply_text(f"⚠️ لقد عدت مبكراً! يرجى الانتظار والمشاهدة بشكل كامل. (حاول بعد {remaining} ثوانٍ)")
        return
