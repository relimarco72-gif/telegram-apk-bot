...
WEBAPP_URL = os.getenv("WEBAPP_URL", "https://ivory-mildrid-44.tiiny.site")
...
def build_channel_message(file_data: dict) -> str:
    ...
    views_progress = create_progress_bar(current_views, required_views) if required_views > 0 else ""

    fully_unlocked = _is_file_fully_unlocked(file_data)

    if fully_unlocked:
        star_status = "✅ مكتمل"
    else:
        star_status = f"⭐ {current}/{total}"

    safe_name = escape_md(name)
    text = (
        f"📦 *{safe_name}*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 تقدم النجوم: {progress}\n"
        f"⭐ النجوم: {star_status}\n"
    )
    ...
    if required_views > 0:
        if fully_unlocked:
            ...
            views_status = f"📹 {current_views}/{required_views}"
        text += (
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 تقدم المشاهدات: {views_progress}\n"
            f"📹 المشاهدات: {views_status}\n"
        )

    text += "━━━━━━━━━━━━━━━━━━━━━━\n"

    if fully_unlocked:
        text += "🎉 تم فتح الملف! شكراً لكم جميعاً!"
    else:
        text += "💫 ادعم بالنجوم أو ساهم بمشاهدة الإعلانات!"

    return text


def build_channel_keyboard(file_key: str, file_data: dict = None):
    """بناء لوحة أزرار رسالة القناة (نجوم + مساهمة جماعية)."""
    # إذا كان الملف مفتوحاً بالكامل، نعرض زر التحميل فقط
    if file_data and _is_file_fully_unlocked(file_data):
        return None  # ستتم إزالة الأزرار وإرسال الملف

    keyboard = []

    # زر الدعم بالنجوم (النظام القديم - بدون تغيير)
    keyboard.append([
        InlineKeyboardButton(
            "⭐ فتح فوري بالنجوم",
    ...
    # زر المساهمة الجماعية عبر إعلانات Monetag
    if file_data:
        current_views = file_data.get("current_views", 0)
        required_views = file_data.get("required_views", 0)
        if required_views > 0 and current_views < required_views:
            keyboard.append([
                InlineKeyboardButton(
                    f"📹 مساهمة جماعية ({current_views}/{required_views})",
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args and len(context.args) > 0:
        ...
        if arg.startswith("support_"):
            ...
            file_key = arg[7:]   # إزالة البادئة "verify_"
            # بدلاً من web_app_data، نعتمد على زر يحولنا للبوت كـ Start للتحقق
...
async def _handle_watch_entry(
    ...
    webapp_url = f"{WEBAPP_URL}?file_key={file_key}"
...
async def _process_views_for_new_file(
    ...
    # نشر الملف في القناة مع شريط التقدم وأزرار الدعم
    ...
    views_text = f"📹 المشاهدات المطلوبة: {required_views}" if required_views > 0 else "📹 الفتح الجماعي: معطّل"
...
async def _handle_view_completed(update: Update, context: ContextTypes.DEFAULT_TYPE, file_key: str) -> None:
    """معالجة إتمام المشاهدة التي تأتي عبر زر (التحقق وإرسال للبوت) من المتصفح."""
    user_id = update.effective_user.id
    ...
    # التحقق السلوكي المتقدم - منع الضغط المتكرر
    uid = str(user_id)
    now = time.time()
    last_view = data["user_last_action"].get(f"{uid}_view_{file_key}", 0)
    
    if now - last_view < 30: # يجب أن تمر على الأقل 30 ثانية بين كل مشاهدة لنفس الملف
        await update.message.reply_text("⚠️ يرجى الانتظار والمشاهدة بشكل كامل قبل التحقق! لا تتسرع.")
        return

    # حفظ وقت المشاهدة الأخيرة
    data["user_last_action"][f"{uid}_view_{file_key}"] = now
    ...
    # عند اكتمال المشاهدات -> فتح الملف للجميع
    if unlocked:
